...
// Fonction pour incrémenterlecompteur devisionnage

void increment_watched(Movies &movies, std::string name) {
    if (movies.increment_watched(name)) {
        std::cout << name << " visionnage incremente" << std::endl;
    } else {
...
void add_movie(Movies &movies, std::string name, std::string rating, int watched) {
    if (movies.add_movie(name, rating, watched)) {
        std::cout << name << " ajoute" << std::endl;
    } else{
        std::cout << name << " existe deja" << std::endl;
    }
    cout << endl;
}
...
int main() {
    ...
    my_movies.display();
    add_movie(my_movies, "Cendrillon", "PG", 7);
    add_movie(my_movies, "L'Age de glace", "PG", 12);
    my_movies.display();
    increment_watched(my_movies, "Big");
    increment_watched(my_movies, "L'Age de glace");
    my_movies.display();
